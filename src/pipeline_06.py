...
def extrair_dados_bitcon():
    """Extrai JSON completo da API da CoinBase"""
...
def salvar_dados_postgres(dados):
    ...
    try:
        ...
        logger.error(f"Erro a inserir dados no PostgreSQL: {ex}")
        session.rollback()
    finally:
        session.close()
...
if __name__ == "__main__":
    criar_tabela()
    logger.info("Iniciando ETL com atuaização a cada 15 segundos... (CTRL+C para interromper)")
    
    while True:
        try:
            dados_json = extrair_dados_bitcon()
            if dados_json:
